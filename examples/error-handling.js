 ...
 * and resilience patterns when using the hookR Client SDK.
 */
...
class ResilientWebhookClient {
  ...
  async initialize() {
    ...
    this.client = new HookRClient(this.clientKey, this.options);
  ...
  setupEventHandlers() {
    ...
    this.client.onConnected((event) => {
      console.log(`✅ Connected to hookR (Client ID: ${event.clientId})`);
...
// Usage example
async function main() {
  const clientKey = process.env.HOOKR_CLIENT_KEY || 'your-client-key-here';
